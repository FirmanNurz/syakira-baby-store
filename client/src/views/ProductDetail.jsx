...
export default function ProductDetail() {
    ...
    async function fetchData() {
        try {
            setLoading(true)
            const {data} = await axios.get(`https://syakira-baby-store.firmanz.tech/products/${id}`)
            ...
            if (token) {
                const favResponse = await axios.get('https://syakira-baby-store.firmanz.tech/favorite', {
    ...
    const handleAddToFavorites = async () => {
        try {
            ...
            if (isFavorite) {
                ...
                if (favorite) {
                    await axios.delete(`https://syakira-baby-store.firmanz.tech/favorite/${favorite.id}`, {
                        headers: { Authorization: `Bearer ${token}` }
                    })
                    setIsFavorite(false)
                }
            } else {
                await axios.post('https://syakira-baby-store.firmanz.tech/favorite', 
